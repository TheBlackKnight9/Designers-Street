...
export default function AdminDashboardPage() {
  ...
  // Active admin section tab: "overview" | "designers" | "products" | "curation" | "editorial"
  const [activeTab, setActiveTab] = useState<"overview" | "designers" | "products" | "curation" | "editorial">("overview");
  ...
  const handleSaveProduct = (e: React.FormEvent) => {
    ...
    if (editingProduct) {
      updateProduct(editingProduct.id, {
        ...
        images: imageList.length > 0 ? imageList : ["https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?w=800&q=80"],
          ...
          <div className="space-y-6">
            ...
            <nav className="space-y-1 text-[10px] font-semibold">
              {[
                ...
                { id: "editorial", label: "Editorial CMS", icon: "📰" },
          ...
          <div>
            ...
            {activeTab === "editorial" && (
              <div className="space-y-5">
                <div className="flex items-center justify-between">
                  <div>
                    <h2 className="text-base font-extrabold uppercase text-[#11161D]">
                      Editorial & Magazine CMS
                    </h2>
                    <p className="text-[10px] text-gray-500">
                      Manage hero campaigns, curated collections, long-form articles, and dynamic home section ordering.
                    </p>
                  </div>
                  <div className="flex gap-2">
                    <button
                      ...
                      onClick={async () => {
                        ...
                        await fetch("/api/dashboard/editorial/campaigns", {
                          method: "POST",
                          ...
                          body: JSON.stringify({
                            ...
                            slug,
                            headline: title,
                            body: "Editorial campaign description.",
                            heroImage: "https://images.unsplash.com/photo-1490481651871-ab68de25d43d?w=1600&q=80",
                          }),
                        });
                        alert("Campaign Created!");
                      }}
                      className="px-3 py-1.5 bg-[#11161D] text-white text-[9px] font-extrabold uppercase rounded-lg shadow-xs"
                    >
                      + New Campaign
                    </button>
                    <button
                      ...
                      onClick={async () => {
                        ...
                        if (!title) return;
                        const slug = title.toLowerCase().replace(/[^a-z0-9]+/g, "-");
                        await fetch("/api/dashboard/editorial/collections", {
                          method: "POST",
                          headers: { "Content-Type": "application/json" },
                          body: JSON.stringify({
                            title,
                            slug,
                            tagline: "Curated seasonal edit",
                            coverImage: "https://images.unsplash.com/photo-1583391733956-3750e0ff4e8b?w=1200&q=80",
                          }),
                        });
                        alert("Collection Created!");
                      }}
                      className="px-3 py-1.5 bg-[#C5A059] text-black text-[9px] font-extrabold uppercase rounded-lg shadow-xs"
                    >
                      + New Collection
                    </button>
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="p-4 bg-white rounded-xl border border-[#E3DBCC] shadow-2xs space-y-2">
                    <span className="text-[9px] font-bold text-[#C5A059] uppercase tracking-wider block">
                      Active Cover Story
                    </span>
                    <h3 className="font-extrabold text-xs text-[#11161D] uppercase">
                      Spring &apos;26 Haute Couture Prelude
                    </h3>
                    <p className="text-[10px] text-gray-500">
                      Featured House: MAISON RIVIÈRE · Status: Published
                    </p>
                    <span className="inline-block px-2 py-0.5 bg-emerald-100 text-emerald-900 rounded font-bold text-[8px] uppercase">
                      Live on Homepage
                    </span>
                  </div>

                  {/* Section Order Config Card */}
                  <div className="p-4 bg-white rounded-xl border border-[#E3DBCC] shadow-2xs space-y-2">
                    <span className="text-[9px] font-bold text-[#C5A059] uppercase tracking-wider block">
                      Homepage Layout Control
                    </span>
                    <h3 className="font-extrabold text-xs text-[#11161D] uppercase">
                      7 Dynamic Sections Active
                    </h3>
                    <p className="text-[10px] text-gray-500">
                      Hero Campaign → House Spotlight → Curated Collections → Articles → Lookbooks → Limited Releases
                    </p>
                    <button
                      type="button"
                      onClick={() => alert("Drag & drop section reordering enabled in production admin.")}
                      className="px-2.5 py-1 bg-gray-100 text-[#11161D] font-bold text-[9px] uppercase rounded-md border border-gray-300 hover:bg-gray-200"
                    >
                      Reorder Sections ⚙️
                    </button>
                  </div>
