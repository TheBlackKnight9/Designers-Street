...
export function productCardToUiProduct(card: ProductCardDTO): Product {
  ...
  return {
    ...
    editionTotal: card.editionTotal ?? undefined,
    editionSold: card.editionSold,
    recentPurchaseCount: card.recentPurchaseCount,
    editorsPick: card.editorsPick,
    handcrafted: card.handcrafted,
    madeToOrder: card.madeToOrder,
    sustainable: card.sustainable,
    badges: card.badges?.length ? card.badges : undefined,
...
export function productDetailToUiProduct(detail: ProductDetailDTO): Product {
  return {
    ...
    careInstructions: detail.careInstructions ?? undefined,
    designerInspiration: detail.designerInspiration ?? undefined,
  };
}
