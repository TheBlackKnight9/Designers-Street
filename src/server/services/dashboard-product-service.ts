...
export class DashboardProductService {
  ...
  private async validatePublishRequirements(ctx: DashboardContext, productId: string) {
    const raw = await this.products.findRawById(productId);
    if (!raw) throw new NotFoundError(`Product ${productId} not found`);
    const media = await this.media.findByProductId(productId);
    ...
    if (raw.piecesRemaining == null && raw.editionTotal == null && !raw.madeToOrder) {
      missing.push("Inventory / Pieces Remaining");
    }
    if (missing.length > 0) {
      throw new ValidationError(`Cannot publish. Complete required fields: ${missing.join(", ")}`);
    }
  }
  ...
  async setStatus(
    ...
    if (status === "published") {
      await this.validatePublishRequirements(ctx, productId);
    }
  ...
  async bulkUpdateStock(
    ...
    const res = await prisma.product.updateMany({
      ...
      data: { piecesRemaining: Math.max(0, Math.round(piecesRemaining)) },
    });
    return { updatedCount: res.count };
  }
}
